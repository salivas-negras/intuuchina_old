...
foreach (array('suffix', 'animation', 'panel', 'title_size', 'center', 'class', 'badge', 'icon', 'display') as $var) {
...
if (in_array($widget->position, array('breadcrumbs', 'logo', 'logo-small', 'search', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
    ...
    if ($animation == 'light') $data = "data-particles='{\"color\":\"#fff\"}'";
    if ($animation == 'dark') $data = "data-particles='{\"color\":\"#000\"}'";
    if ($animation == 'auto') $data = "data-particles";

    echo '<div class="'.implode(' ', $classes).'" '. $data .'>'.$badge.$title.$content.'</div>';
}
